...
export default function Profile() {
  ...
  const { profiles, playerStats, updateProfile } = useLiveMatch();
  ...
  const viewedPlayerName = role === "player" ? (location.state?.playerName || profiles.player.name) : profiles.organizer.name;
  const isViewingParticipant = role === "player" && Boolean(location.state?.playerName);
  const playerRecord = playerStats[viewedPlayerName.trim().toLowerCase()] || { matches: 0, wins: 0, losses: 0, setsWon: 0, setsLost: 0 };
  const playerStatsCards = [
    { icon: Swords, cls: "icon-green", value: playerRecord.matches, label: "Matches Played" },
    { icon: Trophy, cls: "icon-purple", value: `${playerRecord.wins}W - ${playerRecord.losses}L`, label: "Match Record" },
    { icon: Target, cls: "icon-blue", value: playerRecord.matches ? `${Math.round((playerRecord.wins / playerRecord.matches) * 100)}%` : "—", label: "Win Rate" },
    { icon: Trophy, cls: "icon-yellow", value: `${playerRecord.setsWon} - ${playerRecord.setsLost}`, label: "Sets Won / Lost" },
  ];
  const stats = role === "player" ? playerStatsCards : ORGANIZER_STATS;
    ...
    <div className="stat-shell">
      <header className="stat-topbar">
        ...
        <div className="stat-topbar-right">
          ...
          <span className="stat-username">{viewedPlayerName}</span>
        </div>
      </header>
      ...
      <main className="stat-main">
        <div className="dash-header-flex">
          <div>
            <h1>My Profile</h1>
            <p>{isViewingParticipant ? "Match statistics for this tournament participant." : "View and manage your Matcho profile details."}</p>
          </div>
        ...
        <div className="ep-profile-panel">
          <div className="ep-profile-head">
            ...
            <div className="ep-profile-headtext">
              <h2>{viewedPlayerName}</h2>
              <span className="badge-blue">{profiles[role].role}</span>
            </div>
            {!editing && !isViewingParticipant ? (
